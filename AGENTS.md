...
- **Teach Intent**: every chat turn carries a fresh topic roster (name/slug/lesson count) read from disk; the chat agent routes learning intents by emitting a trailing `[TEACH:topic-slug:instructions]` tag. The sidecar strips the tag, speaks the remainder, and dispatches the instructions asynchronously to the topic's persistent teach session (creating the workspace first when the agent confirmed a new topic by voice). Dispatched teach turns always run at a deep reasoning tier (codex `xhigh`, claude `high`) and are instructed to ground the lesson with web search; the chat plane keeps the panel's thinking setting. Failures surface as a spoken `teachError` event.
