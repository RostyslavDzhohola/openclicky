...
| `OverlayWindow.swift` | ~1010 | Full-screen transparent overlay hosting the blue cursor, waveform, spinner, and the typewriter bubbles (pointing phrase + spoken-response read-along). Handles cursor animation, element pointing with bezier arcs, multi-monitor coordinate mapping, and fade-out transitions. |
| `CompanionScreenCaptureUtility.swift` | ~132 | Multi-monitor screenshot capture using ScreenCaptureKit. Returns labeled image data for each connected display. |
| `BuddyDictationManager.swift` | ~1510 | Push-to-talk voice pipeline. Handles microphone capture via `AVAudioEngine`, early-audio buffering while transcription providers start, capture-device enumeration and per-session input pinning via CoreAudio (with default-restore when the pin clears or vanishes), provider-aware permission checks, keyboard/button dictation sessions, transcript finalization, shortcut parsing, contextual keyterms, and live audio-level reporting for waveform feedback. |
...
| `sidecar/index.mjs` | ~1115 | Sidecar entry point: stdin NDJSON dispatch loop, chat-plane routing (roster injection, deterministic CANCEL/OPEN/TEACH tag handling, idle reset), mission-interview state machine (sync first teach turn, routed voice turns, MISSION.md completion check, expiry/turn-cap valves), correlated agent tracing, cancellable async lesson-build dispatch with watcher holds, startup chat/dashboard/watcher bootstrap, teach-template bootstrap, shutdown on stdin EOF. |
...
| `sidecar/src/cancelTag.mjs` | ~27 | Pure `[CANCEL:slug]` parser: strips all cancellation tags from spoken text and returns the first well-formed topic cancellation request. |
| `sidecar/src/openTag.mjs` | ~96 | Pure `[OPEN:slug]` parser and lesson resolver: strips all open tags, normalizes optional ordinals, and finds an existing lesson by filename ordinal. |
| `sidecar/src/teachDispatchRegistry.mjs` | ~110 | Active background-build registry: prevents duplicate per-topic dispatches, coordinates cancellation completion, and quarantines lesson files created during a cancelled dispatch. |
| `sidecar/src/teachCompletion.mjs` | ~18 | Detects lessons created by one dispatch and builds the deterministic user-facing “ready — opening it now” announcement without exposing the agent's raw report. |
...
| `sidecar/src/companionRules.mjs` | ~103 | Single source of truth for the spoken-companion persona + `[POINT:...]`/`[TEACH:slug:instructions]`/`[CANCEL:slug]`/`[OPEN:slug]` tag protocols, roster grounding, and conservative lesson-build routing. Appended to Claude's system-prompt preset; written as AGENTS.md for Codex (full chat notes in `.chat`, slim notes in topic workspaces). |
| `sidecar/src/lessonWatcher.mjs` | ~255 | Chokidar watcher on workspace roots; queues stabilized lessons during reference-counted teach-turn holds, then either discards cancelled-build events or regenerates the dashboard and emits `lessonCreated` with `openedByAgent` computed from the completed turn's shell commands; a 15-minute safety flush prevents stranded notifications. |
...
| `sidecar/src/agentTrace.mjs` / `agentActivity.mjs` | ~115 | Debug-only, bounded JSONL tracing for correlated chat/Teach/interview activity plus privacy-safe Claude sub-agent and Codex capability metadata. |
| `sidecar/test/drive.mjs` | ~598 | Terminal test harness speaking the real protocol: chat/oneshot/auth/workspaces/teach/resume/split/interview drives against live subscriptions, fresh temp dirs per run. Unit tests live in `test/unit/` (`npm test`). |
